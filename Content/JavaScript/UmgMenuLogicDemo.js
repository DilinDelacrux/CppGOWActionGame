...
class UmgMenuLogicDemo extends UE.Actor {
     ...
     */
    MenuWidgetClassPath = DEFAULT_WIDGET_CLASS_PATH;
    Constructor() {
    ...
    openMenu() {
        ...
        if (!widgetClass) {
            console.error(`[UmgMenuLogicDemo] 无法加载 UMG Class：${this.widgetClassPath()}。` +
        ...
        const widget = createdWidget;
        const missingWidgets = this.findMissingWidgets(widget);
        if (missingWidgets.length > 0) {
            console.error(`[UmgMenuLogicDemo] UMG 控件契约不完整：${missingWidgets.join(', ')}。` +
        ...
        const callbacks = {
            ...
            toggleCredits: () => {
                ...
                widget.Panel_Credits.SetVisibility(isVisible ? UE.ESlateVisibility.Collapsed : UE.ESlateVisibility.Visible);
        ...
        UE.WidgetBlueprintLibrary.SetInputMode_UIOnlyEx(controller, widget, UE.EMouseLockMode.DoNotLock, false);
        controller.bShowMouseCursor = true;
        console.warn('[UmgMenuLogicDemo] 方案二启动：外观来自 UMG，交互逻辑来自 TypeScript。');
    }
    // @no-blueprint
    closeMenu(restoreGameInput) {
    ...
    // @no-blueprint
    findMissingWidgets(widget) {
        const missing = [];
        if (!widget.Btn_StartGame || !widget.Btn_StartGame.IsA(UE.Button.StaticClass()))
            missing.push('Btn_StartGame (Button)');
        if (!widget.Btn_QuitGame || !widget.Btn_QuitGame.IsA(UE.Button.StaticClass()))
            missing.push('Btn_QuitGame (Button)');
        if (!widget.Btn_Credits || !widget.Btn_Credits.IsA(UE.Button.StaticClass()))
            missing.push('Btn_Credits (Button)');
        if (!widget.Panel_Credits || !widget.Panel_Credits.IsA(UE.Widget.StaticClass()))
            missing.push('Panel_Credits (Widget)');
        return missing;
    }
}
exports.default = UmgMenuLogicDemo;
//# sourceMappingURL=UmgMenuLogicDemo.js.map