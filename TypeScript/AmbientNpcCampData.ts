/** W1-02 初始数据契约。不依赖 UE，供运行时和离线校验共用。 */
export type ResidentInitialState = { personal_food: number, worship: number, mood: number }
export type CampResident = {
    id: string
    name: string
    profession: string
    profile?: {
    ...
    initial_state?: ResidentInitialState
}
...
export type CampRelationship = {
    from: string, to: string, affection: number, attraction: number, trust: number
}
export type CampWorld = {
    season: 'spring' | 'summer' | 'autumn' | 'winter'
    weather: 'clear' | 'rain' | 'storm' | 'snow'
    resource_abundance: number
    shared_resources: { food: number, water: number, wood: number, herbs: number }
    daily_consumption_per_resident: { food: number, water: number }
...
export type CampState = {
    ...
    name: string
    world: CampWorld
    residents: Record<string, ResidentInitialState>
    relationships: CampRelationship[]
    facts: CampFact[]
}

function requireValue(condition: boolean, message: string): void {
    if (!condition) throw new Error(`[AmbientNpcCamp] ${message}`)
}
...
function resource(value: number, field: string): void {
    requireValue(Number.isSafeInteger(value) && value >= 0, `${field} 必须是非负安全整数`)
}

/** 每次返回新的状态副本。旧的十人日程没有 camp，保持兼容。仅在新会话/显式重置时调用。 */
export function createInitialCampState(daily: { camp?: CampDefinition, npcs: CampResident[] }): CampState | undefined {
    if (daily.camp === undefined) return undefined
    ...
    const residents: Record<string, ResidentInitialState> = Object.create(null)
    for (const npc of daily.npcs) {
        ...
        for (const key of ['personality', 'contribution', 'speech_style', 'faith_attitude'] as const) text(profile[key], `${npc.id}.${key}`)
    ...
    for (const key of ['food', 'water', 'wood', 'herbs'] as const) resource(world.shared_resources[key], `shared_resources.${key}`)
    for (const key of ['food', 'water'] as const) resource(world.daily_consumption_per_resident[key], `daily_consumption_per_resident.${key}`)
