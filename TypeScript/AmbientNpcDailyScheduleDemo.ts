...
type ScheduleJson = {
    ...
    wake_jitter_minutes?: { min?: number, max?: number }
    npcs: ScheduleNpcJson[]
}
...
type ScheduleState = {
    ...
    wakeJitterMin: number
    wakeJitterMax: number
    npcs: Npc[]
}

const scheduleStates = new WeakMap<object, ScheduleState>()
const invalidScheduleInstances = new WeakSet<object>()
...
class AmbientNpcDailyScheduleDemo extends UE.BehaviorFrameworkManagerBase {
    ...
    ReceiveTick(deltaSeconds: number): void {
        ...
        while (schedule.elapsed >= schedule.dayDurationSeconds) {
            ...
            for (const npc of schedule.npcs) npc.state = undefined
            this.applyDailyWakeOffsets(schedule)
        }

        const second = Math.floor(schedule.elapsed)
        if (second === schedule.lastLoggedSecond) return
    ...
    private update(minute: number, schedule: ScheduleState): void {
        ...
        for (let i = 0; i + 1 < relaxing.length; i += 2) {
            ...
            pairs.add(pair)
            if (!schedule.activePairs.has(pair)) console.warn(`[AmbientNpcSchedule] 第 ${schedule.dayIndex + 1} 日 ${time} | 对话开始：${pair}`)
        }
    ...
    private stateAt(npc: Npc, minute: number): State {
        ...
        if ((minute >= npc.wakeTime + npc.wakeOffset && minute < npc.lunchStart) || (minute >= npc.lunchEnd && minute < npc.workEnd)) return '工作'
        if ((minute >= npc.workEnd && minute < npc.dinnerStart) || (minute >= npc.dinnerEnd && minute < npc.sleepStart)) return '放松'
        return '睡觉'
    }
    ...
    private parseTime(value: string): number {
        ...
        if (hour > 23 || minute > 59) throw new Error(`无效的时间：${value}`)
    ...
    private schedule(): ScheduleState | undefined {
        ...
        if (invalidScheduleInstances.has(this)) return undefined
        ...
        try {
            ...
            if (Array.isArray((data.npcs[0] as unknown as { events?: unknown[] }).events)) {
            ...
            if (min < 0 || max < min) throw new Error('wake_jitter_minutes 配置无效')
            ...
            if (schedule.dayDurationSeconds <= 0) throw new Error('day_duration_seconds 必须大于 0')
